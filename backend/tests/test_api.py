...
from models import MealTable, GeneratedRecipeCandidate, RecipeGenerationLog
from schemas import ParsedMeal, ExtractionItem
...
def test_parse_meal_unresolved_audit_flags(client, monkeypatch):
    ...
    def mock_parse(text, client):
        return ParsedMeal(
            meal_type="breakfast",
            items=[ExtractionItem(name="dragonfruit", quantity=1.0, unit="piece", estimated_weight_grams=100.0, raw_or_cooked="raw")]
        )
    monkeypatch.setattr(nutrition_service, "extract_meal_structure", mock_parse)
    monkeypatch.setattr(nutrition_service, "generate_recipe_fallback", lambda *args, **kwargs: None)
    monkeypatch.setattr(nutrition_service, "find_closest_recipe", lambda *args, **kwargs: None)
    monkeypatch.setattr(nutrition_service, "find_closest_food", lambda *args, **kwargs: None)
...
def test_parse_meal_resolved_db_match(client, monkeypatch):
    ...
    monkeypatch.setattr(nutrition_service, "generate_recipe_fallback", lambda *args, **kwargs: None)
    monkeypatch.setattr(nutrition_service, "find_closest_recipe", lambda *args, **kwargs: None)
    monkeypatch.setattr(nutrition_service, "find_closest_food", lambda *args, **kwargs: {"calories": 130.0, "protein": 2.7, "carbs": 28.0, "fat": 0.3, "similarity_score": 0.85})
    ...
    item = data["items"][0]
    assert item["source"] == "food_match_high"
...
def test_parse_meal_low_confidence_match(client, monkeypatch):
    """P2: Verify matches below 0.80 return db_match_low."""
    def mock_parse(text, client):
        return ParsedMeal(
            meal_type="snack",
    ...
    monkeypatch.setattr(nutrition_service, "generate_recipe_fallback", lambda *args, **kwargs: None)
    monkeypatch.setattr(nutrition_service, "find_closest_recipe", lambda *args, **kwargs: None)
    monkeypatch.setattr(nutrition_service, "find_closest_food", lambda *args, **kwargs: {"calories": 100.0, "protein": 1.0, "carbs": 10.0, "fat": 5.0, "similarity_score": 0.65})

    response = client.post("/api/v1/meals/parse", json={"text": "weird snack"})
    assert response.status_code == 200
    item = response.json()["items"][0]
    assert item["source"] == "food_match_low"
...
def test_parse_meal_ambiguity_propagation(client, monkeypatch):
    ...
    monkeypatch.setattr(nutrition_service, "generate_recipe_fallback", lambda *args, **kwargs: None)
    monkeypatch.setattr(nutrition_service, "find_closest_recipe", lambda *args, **kwargs: None)
    monkeypatch.setattr(nutrition_service, "find_closest_food", lambda *args, **kwargs: {"calories": 100.0, "protein": 5.0, "carbs": 15.0, "fat": 2.0, "similarity_score": 0.90})
    ...
    assert item["ambiguity_reason"] == "Vague quantity | Recipe generation failed. Unresolved ingredients: Fallback unavailable"
    assert item["needs_clarification"] is True


def test_parse_meal_failure_handling(client, monkeypatch):
    """P1: Verify ValueError -> 400, and APIError -> 502."""
    def mock_value_error(text, client):
        raise ValueError("Invalid format")
    monkeypatch.setattr(nutrition_service, "extract_meal_structure", mock_value_error)
    monkeypatch.setattr(nutrition_service, "generate_recipe_fallback", lambda *args, **kwargs: None)
    monkeypatch.setattr(nutrition_service, "find_closest_recipe", lambda *args, **kwargs: None)
    ...
    def mock_api_error(text, client):
        raise openai.APIError("OpenAI is down", request=None, body=None)
    monkeypatch.setattr(nutrition_service, "extract_meal_structure", mock_api_error)
